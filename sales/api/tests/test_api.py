...
class Tests(TransactionTestCase):
    ...
    def test_sales_create(self):
        ...
        response = client.post("/api/sales/", json.dumps({**body, "auto":"error"}), content_type='application/json')
        data = response.json()
        self.assertTrue(response.status_code == 404 or response.status_code == 400, msg="Did not get a 404 OK for non-existent auto/")

        response = client.post("/api/sales/", json.dumps({**body, "salesperson":"error"}), content_type='application/json')
        data = response.json()
        ...
        response = client.post("/api/sales/", json.dumps({**body, "customer":"error"}), content_type='application/json')
        data = response.json()
